...
const Term = (props) => {
  ...
  let tabl = data.filter(rw=>rw != null).map((row, ind)=>{
      return <tr key={ind} style={{color:parseInt(row.is_active) == 1 ? 'blue':'red', backgroundColor:parseInt(row.is_active) == 1 ? 'lightgreen':'' }}>
                ...
                <td className='text-center'>
                  <CButton color="dark" size="sm" key={ind} onClick={(item) => history.push(`/setting/3/${props.pid}/${row.id}/0/0/0`)}>
                         <i className="fa fa-cogs"></i> Manage</CButton>
                </td>   
          ...
          <table className="table  table-outline mb-0  d-sm-table">
                <thead className="thead-lighst" style={{backgroundColor:'navy', color:'white'}}>
                  ...
                  <th className="text-center">SN.</th>
                    ...
                    <th className="text-center"><i className='fa fa-cogs'></i> Manage</th>
