...
const Term = (props) => {
  ...
  let tabl = data.filter(rw=>rw != null).map((row, ind)=>{
      return <tr key={ind} style={{color:parseInt(row.is_active) == 1 ? 'red':'blue'}}>
              ...
              <td className='text-center'>
                <a style={{cursor:'pointer'}} onClick={()=>props.onActivate(row.id, row.is_active, props.user.activeschool.id)}><i className={`fa ${parseInt(row.is_active) == 1 ? 'fa-thumbs-down text-danger' : 'fa-thumbs-up text-success'} ml-2 px-2`}></i></a>
